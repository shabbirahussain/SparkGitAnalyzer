...
data <- read.csv('/Users/shabbirhussain/Data/project/mysql-2018-02-01/report/copyPhase/part-00000', sep=",", header = FALSE)
colnames(data) <- c("COMMITS", "FILES", "SUM_COPY_TIME", "CNT_COPY_TIME")

data %>% group_by(round(COMMITS/100, 0), round(FILES/100, 0)) %>% 
    summarise(COPY_TIME = sum(SUM_COPY_TIME), cnt = sum(CNT_COPY_TIME))-> data
data$COPY_TIME <- data$COPY_TIME/data$cnt
colnames(data) <- c("COMMITS", "FILES", "COPY_TIME", "CNT_COPY_TIME")
```



```{r  echo=F, message=F, warning=F, cache=F, results='hide'}
data1 <- subset(data, COMMITS<500 & FILES<1000)
ggplot(data = data1, aes(x=reorder(as.factor(FILES), FILES), y= reorder(as.factor(COMMITS), COMMITS), fill=COPY_TIME)) +
  geom_tile(width=1, height=1) + 
  scale_fill_gradient2(low="green", mid="white", high="red", midpoint=0.25, name="First Copy Stage") +
  labs(x = "# Files", y = "# Commits") + 
...
data <- data1 %>% mutate(V1=ifelse(V1=="com_reg", "Control", "Plugin"))
